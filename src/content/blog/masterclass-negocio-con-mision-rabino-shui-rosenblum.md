---
title: "MASTERCLASS: Negocio con Misión — Emprender desde Fe, Familia y Servicio con el Rabino Shui Rosenblum"
description: "Guía masterclass basada en la entrevista de Café con Emprendedores al Rabino Shui Rosenblum, director de Jabad Venezuela, sobre dinero, fe, Torá, familia, descanso, Venezuela y lanzamiento emprendedor."
pubDate: "2026-06-22"
code: "negocio-con-mision-rabino-shui-rosenblum"
category: "emprendimiento"
tags: ["emprendimiento", "espiritualidad", "torah", "jabad", "venezuela", "fe", "familia", "liderazgo", "cafe-con-emprendedores"]
...
```mermaid
flowchart LR
    A[Misión Personal] --> B[Dinero como Mensajero]
    B --> C[Gestión del Miedo]
    C --> D[Fe y Oración]
    D --> E[Gratitud Diaria]
    E --> F[Shabbat y Descanso]
    F --> G[Familia como Prioridad]
    G --> H[Contexto País]
    H --> I[Inversionista en la Persona]
    I --> J[Ejecución Valiente]
    J --> K[Impacto]
    K --> A
```
...
flowchart LR
    subgraph I_Do["I Do (Instructor)"]
        direction TB
        A1[Misión: convertir dinero en medio de servicio] --> A2[Miedo: distinguir control real de ilusión de control] --> A3[Fe: usar oración y Shemá como ancla] --> A4[Equilibrio: diseñar descanso semanal]
    end
    
    subgraph We_Do["We Do (Colaborativo)"]
        direction TB
        B1[Equipo: construir declaración de misión] --> B2[Interpretar: analizar miedo emprendedor] --> B3[Diseñar: ritual de gratitud y Shabbat] --> B4[Revisar: auditoría familiar del emprendedor]
    end
    
    subgraph You_Do["You Do (Independiente)"]
        direction TB
        C1[Construir: Negocio con Misión Canvas] --> C2[Definir: límites familiares y espirituales] --> C3[Diseñar: plan de lanzamiento con investigación previa] --> C4[Aplicar: ejecutar el primer paso en 7 días]
    end
...
```mermaid
flowchart TD
    A[Talento personal] --> B[Problema real]
    B --> C[Valor entregado]
    C --> D[Ingreso económico]
    D --> E{¿Qué hace el dinero?}
    E -->|Solo consumo| F[Éxito vacío]
    E -->|Servicio y misión| G[Impacto]
    G --> H[Legado]
```
...
```mermaid
flowchart TD
    A[Incertidumbre emprendedora] --> B{Creencia interna}
    B -->|Todo depende solo de mí| C[Ansiedad alta]
    B -->|Hago mi parte y confío| D[Responsabilidad con paz]
    C --> E[Parálisis o control excesivo]
    D --> F[Acción valiente]
    E --> G[Agotamiento]
    F --> H[Claridad]
```
...
```mermaid
flowchart TD
    A[Trabajo intenso] --> B[Gratitud diaria]
    B --> C[Conciencia de bendición]
    C --> D[Menos ansiedad]
    D --> E[Mejores decisiones]
    E --> F[Descanso semanal]
    F --> G[Claridad renovada]
    G --> A
```
...
```mermaid
flowchart TD
    A[Emprendo] --> B[¿Para qué?]
    B --> C[Proveer]
    B --> D[Servir]
    B --> E[Construir legado]
    C --> F[Familia]
    D --> F
    E --> F
    F --> G[Sentido]
...
```mermaid
flowchart TD
    A[Complejidad país] --> B{Lectura}
    B -->|Solo obstáculo| C[Fuga mental o parálisis]
    B -->|Campo de misión| D[Servicio contextual]
    D --> E[Creatividad]
    E --> F[Comunidad]
    F --> G[Empresa resiliente]
```
...
```mermaid
flowchart TD
    A[Idea] --> B[Investigación]
    B --> C[Planificación]
    C --> D[Persona confiable]
    D --> E[Inversionista]
    E --> F[Capital]
    F --> G[Ejecución valiente]
    G --> H[Aprendizaje]
    H --> B
```
